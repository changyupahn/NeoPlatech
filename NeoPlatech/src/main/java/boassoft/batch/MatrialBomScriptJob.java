...
public class MatrialBomScriptJob implements Job {
	...
	public void execute(JobExecutionContext jobContext)
		...
		try {
			...
			batchMysqlInterfaceService.deleteMatrialBom(cmap);

			clist = batchMssqlService.getMartialBOM(cmap);

			if (!clist.isEmpty() && clist.size() > 0) {
				for (int i = 0; i < clist.size(); i++) {
					CommonMap gmap = clist.getMap(i);

					batchMysqlInterfaceService.insertMatrialBom(gmap);
				}
			}
