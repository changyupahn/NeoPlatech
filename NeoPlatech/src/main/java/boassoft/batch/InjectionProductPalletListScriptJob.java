...
public class InjectionProductPalletListScriptJob implements Job {
	...
	public void execute(JobExecutionContext jobContext)
		...
		// setBatchService((BatchService)jobContext.getJobDetail().getJobDataMap().get("batchService"));
				...
				try {
					...
					batchMysqlInterfaceService.deleteInjectionProductPalletList(cmap);
					
					clist = batchMssqlService.getInjectionProductPalletList(cmap);
					
					if (!clist.isEmpty() && clist.size() > 0) {
						for (int i = 0; i < clist.size(); i++) {
							CommonMap gmap = clist.getMap(i);
							
							batchMysqlInterfaceService.insertInjectionProductPalletList(gmap);
						}
					}	
				} catch (Exception e) {
					e.printStackTrace();
