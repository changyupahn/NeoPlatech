...
public class LgGanpanListScriptJob implements Job {
	...
	public void execute(JobExecutionContext jobContext)
		...
		try {
			...
			batchMysqlInterfaceService.deleteLgGanpanList(cmap);
			
			clist = batchMssqlService.getLgGanpanList(cmap);
			
			if (!clist.isEmpty() && clist.size() > 0) {
				for (int i = 0; i < clist.size(); i++) {
					CommonMap gmap = clist.getMap(i);
					
					batchMysqlInterfaceService.insertLgGanpanList(gmap);
				}
			}	
		} catch (Exception e) {
			e.printStackTrace();
