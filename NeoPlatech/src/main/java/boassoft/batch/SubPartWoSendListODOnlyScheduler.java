...
public class SubPartWoSendListODOnlyScheduler {
	...
	public void insertBatchSchdul() throws Exception {
		...
		System.out.println("insertBatchSchdul : "
		...
		jobDetail.setJobClass(SubPartWoSendListODOnlyScriptJob.class);
		jobDetail.setJobDataMap(jobDataMap);

		// Trigger 만들기
		CronTrigger trigger = new CronTrigger(batchId, null, batchCycle);
		System.out.println("배치스케줄을 등록합니다. 배치스케줄ID : " + batchId);
		System.out.println(batchId + " - cronexpression : "
				+ trigger.getCronExpression());
		trigger.setJobName(jobDetail.getName());
		jobDetail.addJobListener(SubPartWoSendListODOnlyListener.class
				.getName());
		...
		try {
			...
			System.out.println("스케줄러에 배치작업추가할때 에러가 발생했습니다. 배치스케줄ID : "
					+ batchId);
	...
	public void init() throws Exception {
		...
		// Set up the listener
		SubPartWoSendListODOnlyListener listener = new SubPartWoSendListODOnlyListener();
