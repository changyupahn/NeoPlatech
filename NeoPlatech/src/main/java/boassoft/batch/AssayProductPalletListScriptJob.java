...
public class AssayProductPalletListScriptJob implements Job {
	...
	public void execute(JobExecutionContext jobContext)
		...
		try {
			...
			batchMysqlInterfaceService.deleteAssayProductPalletList(cmap);
			
			clist = batchMssqlService.getAssayProductPalletList(cmap);
			...
			if (!clist.isEmpty() && clist.size() > 0) {
				for (int i = 0; i < clist.size(); i++) {
					...
					batchMysqlInterfaceService.insertAssayProductPalletList(gmap);
				}
		    }
		} catch (Exception e) {
			e.printStackTrace();
