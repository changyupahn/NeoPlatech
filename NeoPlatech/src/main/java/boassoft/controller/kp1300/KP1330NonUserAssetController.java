...
@Controller
public class KP1330NonUserAssetController {
	...
	/** log */
    protected static final Log LOG = LogFactory.getLog(KP1330NonUserAssetController.class);

    @RequestMapping(value="/kp1300/kp1330.do")
	public String kp1330(HttpServletRequest request, HttpServletResponse response, ModelMap model) throws Exception {
    	...
    	return "kp1300/kp1330";
	}

    @RequestMapping(value="/kp1300/kp1330Ajax.do")
	public String kp1330Ajax(HttpServletRequest request, HttpServletResponse response, ModelMap model) throws Exception {
    	CommonMap cmap = new CommonMap(request);
    ...
    @RequestMapping(value="/kp1300/kp1330Excel.do")
	public String kp1330Excel(HttpServletRequest request, HttpServletResponse response, ModelMap model) throws Exception {
    	CommonMap cmap = new CommonMap(request);
    	cmap.put("pageIdx", cmap.getString("pageIdx", "1"));
    	...
    	String[] headerListLgc1 = new String[headerSize + 1];
    	String[] headerListLgc2 = null;
    	String[] headerListPhc = new String[headerSize + 1];
    	String[] headerListTyp = new String[headerSize + 1];
    	String[] headerListWidth = new String[headerSize + 1];
    	String[][] mergedRegion = null;
    	int idx = 0;

    	headerListLgc1[0] = "미배정사유";
		headerListPhc[0] = "nosetReason";
		headerListTyp[0] = "TEXT";
		headerListWidth[0] = "" + Math.round(150 / 10);

    	while (idx<dispMngList.size()) {
    		CommonMap dispMng = dispMngList.getMap(idx);
			headerListLgc1[idx+1] = dispMng.getString("logical_name");
			headerListPhc[idx+1] = dispMng.getString("physical_name");
			headerListTyp[idx+1] = dispMng.getString("data_disp_type");
			headerListWidth[idx+1] = "" + Math.round(dispMng.getInt("default_width",100) / 10);
			idx++;
    	}

    	ExcelUtil.write2(request, response, resultList, "미배정자산목록", headerListLgc1, headerListLgc2, headerListPhc, headerListTyp, mergedRegion, headerListWidth, 20);

    	return null;
	}

    /** 부외전환처리 */
    @RequestMapping(value="/kp1300/kp1330Proc.do")
	public String kp1330Proc(HttpServletRequest request, HttpServletResponse response, ModelMap model) throws Exception {
    	CommonMap cmap = new CommonMap(request);
    	...
    	try {
	    	...
	    	//저장
	    	resultCnt = assetService.updateAssetOut(cmap, paramList);
