 ...
 * 달력 관련 클래스.
 *
 * 날자에 관한 핸들링을 쉽게하기 위해 Calendar class를 Wrapping 하였으며,
 * 자주 사용되는 기능들을 구현하였다.
 *
 * @author 고은배
 */
...
public class CalendarUtil {
	  ...
	  private int year;			/** 년도 */
	    ...
	    /**
	     * 현재 시스템의 시간을 반환한다.
	     * @return int   현재 시스템의 시간 (HHMMDD)
	     */
	    public int getTime() {
	        return Integer.parseInt(dFormat.format(this.hour) +
	                                dFormat.format(this.minute) +
	                                dFormat.format(this.second));
	    }

	    /**
	     * 현재 시스템의 시간을 반환한다.
	     * @return String 현재 시스템의 시간 (HH:MM:DD와 같은)
	     */
	    public String getTime(String delim) {
	        return dFormat.format(this.hour) + delim +
	               dFormat.format(this.minute) + delim +
	               dFormat.format(this.second);
	    }

	    /**
	     * 현재 시스템의 년도를 반환한다.
	     * @return int   현재 시스템의 년도
	     */
	    public int getYear() {
	        return this.year;
	    }
	    ...
	    /**
	     * 현재 시스템의 요일을 반환한다.
	     * @return int   현재 시스템의 요일 (일요일:1 ~ 토요일:7)
	     */
	    public int getWeek() {
	        return this.week;
	    }
	    ...
	    public String getWeek(String year, String month, String day) {
	        ...
	        return "" + getWeek(year1, month1, day1);
	    }

	    /**
	     * 현재 시스템의 일자가 해당 월에서 몇 주차인지를 반환한다.
	    ...
	    /**
	     * 현재 시스템의 일자가 포함된 주의 첫번째 날짜를 반환한다.
	     * @return int[]  년, 월, 일의 배열
	     */
...
//	    }
	    ...
	    /**
	     * 입력된 년도와 월에 해당하는 전체 일과 그에 해당하는 요일을 찾아 반환한다.
	     * @return  String[][] 해당 월의 전체 일 및 요일(1차원 배열 : 일, 2차원 배열 : 요일)
	     */
	    ...
	    public int[][] getDaysOfWeek() {
	        ...
	        for (int i=0; i<7; i++) {
	            ...
	            if (startDay > lastDayOfMonth) {
	                startDay = 1;
	                /* 현재 달이 12월일 경우 새로운 년도의 1월로 설정한다. */
	                if (weekMonth == 12) {
	                    weekMonth = 1;
	                }
	                else {
...
//	        fromDay  = Integer.parseInt(dateFull.substring(6,8));
	//
//	        fromWeek = calHelper.getWeek( fromYear, fromMonth, fromDay );
	//
//	        //인수로 받은 날짜로 세팅한다.
...
//	        firstDayFull = format.dateFormat( fromYear, fromMonth, fromDay, "" );
	//
//	        //System.out.println("시작날짜: " +  firstDayFull);
	//
//	        //마지막날짜 계산해서 배열에 저장
...
//	        //System.out.println("마지막날짜: " +  lastDayFull);
//	    }
	    ...
	    /**
	     * 현재 시스템 날짜에 해당하는 달의 달력을 배열로 반환
	     *
	     * @return 달력 모양의 2차원 배열
	     * @author Cholhee Lee
	     */
	    ...
	    public int[][] getCalendar(int year, int month)
	    {
	        ...
	        int maxWeek = getWeekOfMonth(year, month, maxDay);
	        int[][] myCalendar = new int[maxWeek][7];
	        ...
	        for (int d = 1; d <= maxDay; d++) {
	            int row = getWeekOfMonth(year,month,d)-1;
	            int col = getWeek(year,month,d)-1;
	            myCalendar[row][col] = d;
	        }
