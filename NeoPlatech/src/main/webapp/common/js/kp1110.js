...
function fnInitTreeData(){
	...
	//그룹 트리 구조 생성
	$('#treeData input[name="kp1110TreeData"]').each(function(){
...
function fnDetailTreeData(seq) {
	$.ajax({
		type : "POST",
		url : "/kp1100/kp1110Ajax.do",
		data : {
			sItemType : $('#sItemType').val(),
			itemSeq : seq
		},		
...
function fnRegTreeData() {
	...
	$('#sItemType').val('DEFAULT');
	$('#sItemSeq').val('0000');
	
	$.ajax({
		type : "POST",
		url : "/kp1100/kp1110RegAjax.do",
		data : {
			sItemType : $('#sItemType').val(),
			sItemSeq : $('#sItemSeq').val()
		},		
		success:function(data)		
		{
			$('#contDiv').html(data);
