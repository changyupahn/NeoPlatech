...
function fnInitTreeData(){
	...
	//그룹 트리 구조 생성
	$('#treeData input[name="kp9011TreeData"]').each(function(){
		...
		if( spt.length > 6 ){
			...
			if( $('#KP_TREE_'+spt[0]).length == 0 ){
				//root check1
				if( spt[2] == "" ){
					$('#tree').append('<ul><li id="KP_TREE_'+ spt[0] +'" customcode="" customny="" customdc="" customatc="" rel="root"><a href="#" title="'+ spt[1] +'">'+ spt[1] +'</a></li></ul>');
				}
				//parent
				...
				//this
				$('#KP_UL_'+ spt[2]).append('<li id="KP_TREE_'+ spt[0] +'" customcode="'+ spt[3] +'" customny="'+ spt[5] +'" customdc="'+ spt[6] +'" customatc="'+ spt[7] +'" rel="'+ (spt[4]=="Y"?"folder":"delete") +'"><a href="#" title="'+ spt[1] +'">'+ spt[1]+'</a></li>');
			}
		}
	});
}
...
function fnInitTree(nodeOpenYn){
	...
	jstree.bind("loaded.jstree", function (event, data) {
		...
		if ($('#searchKeyword').val() != "" || $('#sItemType').val() == "ZEUS") {
			//전체 트리 오픈
			data.inst.open_all();
		} else {
			//1depth 트리 오픈
	...
	jstree.bind("select_node.jstree", function (event, data) {
		...
		var cd = data.rslt.obj.attr("customcode");
		var nm = data.inst.get_text(data.rslt.obj);
		var ny = data.rslt.obj.attr("customny");
		var dc = data.rslt.obj.attr("customdc");
		var atc = data.rslt.obj.attr("customatc");
