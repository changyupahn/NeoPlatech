...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		var p = $.extend(true,{
			...
			cmTemplate : {},
			idPrefix : "",
			/* Body영역의 RowSpan 처리여부 */ 
			isTableRowSpan : false,
			tableRowSpanTgt:[],
			/* Only one select 용 옵션 추가 */
			oneSelect:false,
			/* Selection 스타일 적용 여부 Default 적용*/
			isSelectionStyle:true,
			/* isCellRwChk 를 사용하는 경우 사용자 Page에 getCellEditable 구현되어야 함 */
			isCellRwChk:false,
			/* 이미선택된 ID목록 */
			selectionIds : [],
			/* Table의 Body영역에 대한 Drag&Drop를 통한 선택 옵션 */
			areaSelect : false,
			isTdIds:false,
			/* RowSpan관련 TD ID */
			rowspanTdId : [],
			/* ColSpan관련 TD ID */
			colspanTdId : [],
			/* MultiKey 설정값 무시 그리드에서 multiselect(Multikey설정) 체크시에도 SetSelection 처리되도록 변경 */
			isMultikeyDisregard:false,
			selbeforerowid:null,
			/* Head의 checkbox 제어시 hidden Row는 처리에서 제외 */
			isCheckedHideExcl : false,
			/* Grid의 출력결과에 합계, 평균 등의 Row를 추가하는 경우 */
			isSum : false,
			isEnterCellFix:false,
			isSinglPage:false,
			isPageKor:false,
			/* 퍼미션 확인용 (자바에서 리턴되는 Json에 추가해야 사용가능)  */
			permAt:"Y",
			exclusionSpecCharList:[]
		}, $.jgrid.defaults, pin || {});
		var grid={
			headers:[],
			cols:[],
			...
			dragEnd: function() {
				...
				if(this.resizing) {
					...
					if(p.forceFit===true){
						...
						$('table:first',this.bDiv).css("width",$.browser.webkit || $.browser.safari ? (p.tblwidth+1) : p.tblwidth +"px");
		...
		formatCol = function (pos, rowInd, tv, rawObject, rowId, rdata){
			...
			if( ts.p.areaSelect )
			{
				result += (clas !== undefined ? (" class=\" drop "+clas+"\"") :"class=\"drop\"") + ((cm.title && tv) ? (" title=\""+$.jgrid.stripHtml(tv)+"\"") :"");
			}
			else
			{
		...
		/*addCell = function(rowId,cell,pos,irow, srvr, beforeCell) {			
			var v,prp,tmp;
			var isRowSpan = false;
			var cellTmp;			
			
			 * Body 영역 출력중 Text가 동일한 Row에 대한 RowSpan 처리용
			...
			v = formatter(rowId,cell,tmp,srvr,'add');			
			
			if( ts.p.isTableRowSpan )
			{	
				if( ts.p.areaSelect )
				{	
					isRowSpan = getChkRowSpan(tmp);
					prp = formatCol( pos,irow, v, srvr, rowId, true);
					if( ts.p.isTdIds )
					{
						if( isRowSpan )
						{
							if( v.indexOf("BLANK") != -1 )
							{
								if( v.indexOf("YEARBLANK") != -1 )
								{
									cellTmp = "<td id=\"" + irow + "_" + pos + "\" role=\"gridcell\" "+prp+" class=\"ui-jqgrid-left-non\"></td>";								
								}
								else
								{
									if( tmp < ts.p.tableRowSpanTgt.length-1 && v != beforeCell)
										cellTmp = "<td id=\"" + irow + "_" + pos + "\" role=\"gridcell\" "+prp+" class=\"ui-jqgrid-left-non\"></td>";
									else if( tmp < ts.p.tableRowSpanTgt.length-1 && v == beforeCell)
									{
										cellTmp = "<td id=\"" + irow + "_" + pos + "\" role=\"gridcell\" "+prp+" class=\"ui-jqgrid-left-non\"></td>";
										$("#" + (irow-1) + "_"+ pos).addClass("ui-jqgrid-bottom-non");
									}
									 tableRowSpanTgt 마지막 Cell은 RowSpan 미처리 
									else
										cellTmp = "<td id=\"" + irow + "_" + pos + "\" role=\"gridcell\" "+prp+" class=\"ui-jqgrid-left-non\"></td>";
								}
							}
							else
							{
								if( v == beforeCell )
								{
									if( tmp < ts.p.tableRowSpanTgt.length-1)
									{
										cellTmp = "<td id=\"" + irow + "_" + pos + "\" role=\"gridcell\" "+prp+"></td>";
										//alert("id : " + "#" + (irow-1) + pos + " val : " + $("#" + (irow-1) + pos).text());
										$("#" + (irow-1) + "_"+ pos).addClass("ui-jqgrid-bottom-non");
									}
									 tableRowSpanTgt 마지막 Cell은 RowSpan 미처리 
									else
										cellTmp = "<td id=\"" + irow + "_" + pos + "\" role=\"gridcell\" "+prp+">"+v+"</td>";
								}
								else
								{
									cellTmp = "<td id=\"" + irow + "_" + pos + "\" role=\"gridcell\" "+prp+">"+v+"</td>";
				...
				else
				{
					...
					if( isRowSpan )
					{
						if( v.indexOf("BLANK") != -1 )
						{
							if( v.indexOf("YEARBLANK") != -1 )
							{
								cellTmp = "<td role=\"gridcell\" "+prp+" class=\"ui-jqgrid-left-non\"></td>";
							}
							else
							{
								if( tmp < ts.p.tableRowSpanTgt.length-1 && v != beforeCell)
									...
									cellTmp = "<td role=\"gridcell\" "+prp+" class=\"ui-jqgrid-left-non ui-jqgrid-top-non\"></td>";
								 tableRowSpanTgt 마지막 Cell은 RowSpan 미처리 
								else
									cellTmp = "<td role=\"gridcell\" "+prp+" class=\"ui-jqgrid-left-non\"></td>";
							}
						...
						else
						{
							...
							{
								if( tmp < ts.p.tableRowSpanTgt.length-1)
									cellTmp = "<td role=\"gridcell\" "+prp+" class=\"ui-jqgrid-top-non\"></td>";
								 tableRowSpanTgt 마지막 Cell은 RowSpan 미처리 
								else
									cellTmp = "<td role=\"gridcell\" "+prp+">"+v+"</td>";
							}
							else
							{
								cellTmp = "<td role=\"gridcell\" "+prp+">"+v+"</td>";
		 ...
		 * Body 영역 출력중 Text가 동일한 Row에 대한 RowSpan 처리용
		 */
		getChkRowSpan = function(j) 
		{
			for( var k = 0; k < ts.p.tableRowSpanTgt.length; k++ )
			{
				if( j == ts.p.tableRowSpanTgt[k] )
					return true;
			}
			return false;
		},
		addJSONData = function(data,t, rcnt, more, adjust) {
			...
			var ir=0,v,i,j,f=[],F,cur,gi=0,si=0,ni=0,len,drows,idn,rd={}, fpos, idr,rowData=[],rowDataBefore=[],rowDataAfter=[],cn=(ts.p.altRows === true) ? " "+ts.p.altclass:"",cn1,lp;
			ts.p.page = $.jgrid.getAccessor(data,dReader.page) || 0;
			lp = $.jgrid.getAccessor(data,dReader.total);
			ts.p.lastpage = lp === undefined ? 1 : lp;
			ts.p.records = $.jgrid.getAccessor(data,dReader.records) || 0;
			ts.p.userData = $.jgrid.getAccessor(data,dReader.userdata) || {};
			ts.p.permAt = $.jgrid.getAccessor(data,dReader.permAt) || "N";
			...
			while (i<len) {
				...
				for (j=0;j<F.length;j++) {
					...
					if( ts.p.isTableRowSpan && ts.p.tableRowSpanTgt.length > 0 )
					{
						if( idr == 1 )
						{
						...
						else
						{
							...
							if( ts.p.multiselect )
								rowData.push( addCell(idr,v,j+gi+si+ni,i+rcnt,cur, rowDataBefore[(j+gi+si+ni-1)]));
							else
								rowData.push( addCell(idr,v,j+gi+si+ni,i+rcnt,cur, rowDataBefore[(j+gi+si+ni)]));
			...
			/*
			var tmpTr = $("#"+$.jgrid.jqID(ts.p.id)+" tbody:first tr:eq(" + (i+rcnt) + ") ");
			$("td", tmpTr).each(function(idx, val) {
				$(this).mouseup(function(e) {
					alert("mouseup");
					ts.p.endRowIdx = (i+rcnt);
					ts.p.endColIdx = idx;
				}).mousedown(function(e) {
					ts.p.startRowIdx = (i+rcnt);
					ts.p.startColIdx = idx;
		...
		updatepager = function(rn, dnd) {
			...
			{
				if( ts.p.noRecordChk ) {
					$(".ui-paging-info",pgboxes).html($.jgrid.format(ts.p.recordtext, ts.p.isPageKor ? 0 : ts.p.emptyrecords));
				}
				else {
					$(".ui-paging-info",pgboxes).html($.jgrid.format(ts.p.recordtext, ts.p.isSum ? ts.p.records - 1 : ts.p.records));
				}
			}
			else
			{
				if(pgboxes) {
					...
					if (ts.p.viewrecords){
						if(ts.p.reccount === 0 || ts.p.noRecordChk ) {
							$(".ui-paging-info",pgboxes).html( ts.p.isPageKor ? ($.jgrid.format(ts.p.recordtext, 0, 0, 0)) : ts.p.emptyrecords);
						} else {
							from = base+1;
							tot=ts.p.records;
							if($.fmatter) {
		...
		setPager = function (pgid, tp){
			...
			$('.ui-pg-selbox',"#"+pgcnt).bind('change',function() {
				/* 페이지당 출력건수 변경시 page를 1로 재설정 처리 조현기 */
				ts.p.page = 1;
		...
		for(i=0;i<this.p.colNames.length;i++){
			...
			/*
			 * ColName의 값이 'BLANK' 인 경우 공백으로 변경 및 Top의 Boarder Hidden 처리 
			 */
			var colHeaderTitle = ts.p.colNames[i];
			if( colHeaderTitle == "BLANK" )
			{
				colHeaderTitle = "";
			}
			
			if( colHeaderTitle.indexOf("<BR>") != -1 )
			{
				intHeaderWithBrTag = colHeaderTitle.split("<BR>").length;
			}
			else
				intHeaderWithBrTag = 0;
			
			var tooltip = ts.p.headertitles ? (" title=\""+$.jgrid.stripHtml(colHeaderTitle)+"\"") :"";
			if( colHeaderTitle == "" )
				thead += "<th id='"+ts.p.id+"_"+ts.p.colModel[i].name+"' role='columnheader' class='ui-state-default ui-th-column ui-th-"+dir+"'"+ tooltip+" style='border-top:0px;'>";
			else
			{
				if( intHeaderWithBrTag > 0 )
				{
					/* ColName에 해당하는 Title 출력시 height : auto 로 변경 */
					//thead += "<th id='"+ts.p.id+"_"+ts.p.colModel[i].name+"' role='columnheader' class='ui-state-default ui-th-column ui-th-"+dir+"'"+ tooltip+" style='word-wrap:break-word;height:" + (headerHeightDefault * intHeaderWithBrTag) + ";'>";
					thead += "<th id='"+ts.p.id+"_"+ts.p.colModel[i].name+"' role='columnheader' class='ui-state-default ui-th-column ui-th-"+dir+"'"+ tooltip+" style='word-wrap:break-word;height:auto;'>";
				}
				else
				{
					thead += "<th id='"+ts.p.id+"_"+ts.p.colModel[i].name+"' role='columnheader' class='ui-state-default ui-th-column ui-th-"+dir+"'"+ tooltip+" style='word-wrap:break-word;'>";					
				}
			}
			...
			{
				/* ColName에 해당하는 Title 출력시 height : auto 로 변경 */
				//thead += "<div id='jqgh_"+ts.p.id+"_"+ts.p.colModel[i].name+"' "+tdc+" style='word-wrap:break-word;height:" + (headerHeightDefault * intHeaderWithBrTag) + ";'>"+colHeaderTitle;
				thead += "<div id='jqgh_"+ts.p.id+"_"+ts.p.colModel[i].name+"' "+tdc+" style='word-wrap:break-word;height:auto;'>"+colHeaderTitle;
			}	
			else
			{
				thead += "<div id='jqgh_"+ts.p.id+"_"+ts.p.colModel[i].name+"' "+tdc+" style='word-wrap:break-word;'>"+colHeaderTitle;					
			}
		...
		$(this).append(thead);
		if( intHeaderWithBrTag > 0 )
			$("thead tr:first th",this).css("height", "auto");
					...
					$(ts.rows).each(function(i) {						
						if (i>0) {
							if(!$(this).hasClass("ui-subgrid") && !$(this).hasClass("jqgroup") && !$(this).hasClass('ui-state-disabled')){
								if( !ts.p.isCheckedHideExcl || (ts.p.isCheckedHideExcl && $(this).css("display") != "none") )
								{									
									if( !ts.p.isFstChkVisible || (ts.p.isFstChkVisible && $("#jqg_"+$.jgrid.jqID(ts.p.id)+"_"+$.jgrid.jqID(this.id), ts.grid.fbDiv ).css("display") != "none") ) {
								    ...
								    if( !ts.p.oneSelect )
									{
								    	if( !ts.p.isMultikeyDisregard )
								    		$(this).addClass("ui-state-highlight").attr("aria-selected","true");
									}
								    if( !ts.p.isFstChkVisible ) {
										...
										if( $("#jqg_"+$.jgrid.jqID(ts.p.id)+"_"+$.jgrid.jqID(this.id), ts.grid.fbDiv ).css("display") != "none" ) {
											ts.p.selarrrow.push(this.id);
											ts.p.selrow = this.id;
										}
		...
		$("th",thr).each(function ( j ) {
			...
			if( !(nm == 'cb' || nm=='subgrid' || nm=='rn') ) {
				if( typeof ts.p.colModel[j].ischeckbox == "undefined" || !ts.p.colModel[j].ischeckbox ) {
		...
		$(ts).before(grid.hDiv).click(function(e) {
			...
			var tableUiJqgridBtable = $(td,ts).closest("table.ui-jqgrid-btable");
			var tableUiJqgridBtableId =(tableUiJqgridBtable.length > 0)? $(td,ts).closest("table.ui-jqgrid-btable")[0].id : "";
			...
			if($(ptr).length === 0 || ptr[0].className.indexOf( 'ui-state-disabled' ) > -1 || tableUiJqgridBtableId.replace("_frozen","") !== ts.id ) {
				return this;
			}
				...
				if(ts.p.cellEdit === true) {
					...
					if(ts.p.isMultikeyDisregard )
					{
						if( ts.p.multiselect && scb) {
							...
							$(ts).jqGrid("setSelection",ptr[0].id,true, true, false);
						}
						else if( ts.p.multiselect && !e[ts.p.multikey] ) {
							/* multiselect가 설정되어 있지만 row를 click시 selection style만 적용되도록 변경 */
							$(ts).jqGrid("setSelection",ptr[0].id, false, false);
						} 
...
$.jgrid.extend({
	...
	setGridWidth : function(nwidth, shrink) {
		return this.each(function(){
			...
			//	그루핑 헤더 리사이즈 버그 수정 13.10.31
				var row$ = $('tr[role=row]>th', $t.grid.hDiv),
				isRow = row$.length > 0;
				...
				$.each($t.p.colModel, function(i) {
					if(this.hidden === false && !this.fixed){
						...
						if(cle) {
							...
							if(isRow) row$[i].style.width = (cw+1)+"px";
						}
						lvc = i;
					}
				});
	...
	clearGridData : function(clearfooter) {
		return this.each(function(){
			var $t = this;
			if ($t.p.noRecordText) $($t).setGridParam({noRecordChk:$($t).getGridParam('records') > 1}).noRecords();
			if(!$t.grid) {return;}
	...
	$.fmatter.util = {
		...
		NumberFormat : function(nData,opts) {
			...
			if(!$.fmatter.isNumber(nData)) {
				/*
				 * 보고통계 수치입력시 문자열 입력 허용을 위해 아래 로직 추가 (원본 nData *= 1;)
				 */
				var nDataTmp = nData;
				nData *= 1;
				if( isNaN(nData) )
...
$.jgrid.extend({
	...
	noRecords:function() {
		return this.each(function(){
			var this$ = $(this);
			if( this$.getGridParam('noRecordText') && !this$.getGridParam('noRecordChk') && this$.getGridParam('records') == 0 ) {	
				this$.setGridParam({ 'noRecordChk' : true }).addRowData(-1, {});
				
				this$.find('tr#-1').empty()
				.append($('<td style="text-align:center; font-weight:bold;">').attr('colspan', this$.find('tr.jqgfirstrow>td:not([style*=display])').size())
				//.text('조회된 데이터가 없습니다.'));
				.text(''));
			}
			else if( this$.getGridParam('noRecordText') && this$.getGridParam('noRecordChk') && this$.getGridParam('records') > 1 ) {
				this$.setGridParam({ noRecordChk : false }).delRowData(-1);
			}			
		});
...
$.jgrid.extend({
	...
	addChildNode : function( nodeid, parentid, data ) {
		...
		if(data) {
			...
			var prow;
			// 원본은 아래의 소스이나 parentid가 null 인경우 getInd에서 예외발생됨
