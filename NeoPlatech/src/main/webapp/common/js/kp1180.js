...
function fnInitTreeData(){
	...
	//그룹 트리 구조 생성
	$('#treeData input[name="kp1180TreeData"]').each(function(){
...
function fnDetailTreeData(seq) {
	$.ajax({
		type : "POST",
		url : "/kp1100/kp1180Ajax.do",
		data : {
			sBldngType : $('#sBldngType').val(),
			bldngSeq : seq
		},		
...
function fnRegTreeData() {
	...
	$('#sBldngType').val('DEFAULT');
	$('#sBldngSeq').val('0000');
	
	$.ajax({
		type : "POST",
		url : "/kp1100/kp1180RegAjax.do",
		data : {
			sBldngType : $('#sBldngType').val(),
			sBldngSeq : $('#sBldngSeq').val()
		},		
		success:function(data)		
		{
			$('#contDiv').html(data);
