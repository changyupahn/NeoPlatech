...
<%
String pageTitle = "불용처분및폐기 (승인별)";
String curAction = "/kp1500/kp1510.do";
String curGridAction = "/kp1500/kp1510Ajax.do";
...
var colNames01 = [
                'rqstno','신청일자','승인일자','처리일자','신청부서','신청자','자산건수','승인상태'
                <% if (ssAuthManager) { %>
                ,'불용처리'
                <% } %>
  				];
var colModel01 = [
  				...
  				<% if (ssAuthManager) { %>
				,{name:'관리버튼', index:'관리버튼', align:'CENTER', width:'150px', columntype:'text', classes:'grid-col-TEXT', sortable:false, editable:false,
					formatter: function(value, options, rData)
					{
						if (value == null) value = "";
						value += ' <span class="button2"><input type="button" value="불용처리" onclick="fnDisusePop(\''+ rData.rqstno +'\');"></span>';
						return value;
					}}
				<% } %>
  				];
...
function fnDisusePop(rqstno) {
	$('#layerPop').click();
	$('#iframe').attr("src", "/kp1500/kp1512.do?rqstno="+rqstno);
	$('#layer_iframe').show();
}
function fnDisusePop2(rqstno) {
	$('#layerPop').click();
...
<body>
	...
	<td width="50%" style="text-align: right;">
		...
		<%-- <% if (ssAuthManager) { %>
		<span class="button"><input type="button" value="불용처리" onclick="fnDisusePop2();"></span>
		<% } %> --%>
		&nbsp;
	</td>
	</tr>
