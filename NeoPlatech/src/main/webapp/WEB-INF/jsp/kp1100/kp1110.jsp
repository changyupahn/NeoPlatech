...
<%
String pageTitle = "물품분류관리";
String curAction = "/kp1100/kp1110.do";
String curExcel = "/kp1100/kp1110Excel.do";
String curProc = "/kp1100/kp1110Proc.do";
String curDelProc = "/kp1100/kp1110DelProc.do";
...
<script type="text/javascript" src="/common/js/kp1110.js?<%=new java.text.SimpleDateFormat("yyyyMMddHHmmss").format(new java.util.Date())%>"></script>
	...
	if( confirm("삭제 하시겠습니까?") ){
		$.ajax({
			...
			success:function(data)
			{
				if (data.ret == "OK") {
					alert("삭제되었습니다.");
					location.href = "<%=curAction%>?sItemType=" + $('#sItemType').val();
				} else {
					alert(data.retmsg);
				}
			},
...
function fnSave(){
	if( confirm("저장 하시겠습니까?") ){
		...
		$.ajax({
			...
			url : "<%=curProc%>",
			data : {
				parentItemSeq : $('#parentItemSeq').val()
...
<body>
	...
	<input type="hidden" id="sItemSeq" name="sItemSeq" value="" />
	...
	<div id="treeData">
		<input type="hidden" name="kp1110TreeData" value="0000|물품분류||0|Y" />
		...
		for (int i=0; i<assetTypeCdList.size(); i++) {
			...
			%>
			<input type="hidden" name="kp1110TreeData" value="<%=seq%>|<%=codeName%>|<%=pseq%>|0|<%=useYn%>" />
			<%
		}
		--%>
		...
		for(int i=0; i<dataList.size(); i++){
			...
			String pseq = tree.getString("parentItemSeq","0000").trim();
