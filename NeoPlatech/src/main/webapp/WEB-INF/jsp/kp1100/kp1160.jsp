...
<%
String pageTitle = "제조업체코드관리";
String curAction = "/kp1100/kp1160.do";
String curExcel = "/kp1100/kp1160Excel.do";
String curProc = "/kp1100/kp1160Proc.do";
String curDelProc = "/kp1100/kp1160DelProc.do";
...
<script type="text/javascript" src="/common/js/kp1160.js"></script>
...
function fnSetKp9050(obj) {
	$('#natnCd').val(obj.natnCd);
	$('#natnName').val(obj.natnName);
}
</script>
...
<body>
	...
	<div id="treeData">
		<input type="hidden" name="kp1160TreeData" value="0|제조업체코드||0|Y" />
		<%
		for(int i=0; i<dataList.size(); i++){
			CommonMap tree = dataList.getMap(i);
			String seq = tree.getString("compSeq");
			String code = tree.getString("compCd");
			String codeName = "[" + code + "] " + tree.getString("compName");
			String pseq = tree.getString("parentCompSeq").trim();
			String useYn = tree.getString("useYn");

			if ("0000".equals(code)) {
				codeName = tree.getString("compName");
			}
		%>
		<input type="hidden" name="kp1160TreeData" value="<%=seq%>|<%=codeName%>|<%=pseq%>|0|<%=useYn%>" />
		<%
		}
		%>
	</div>
