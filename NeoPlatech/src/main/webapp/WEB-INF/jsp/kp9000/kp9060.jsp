...
<%
String pageTitle = "제조업체선택";
String curAction = "/kp9000/kp9060.do";
String curGridAction = "/kp9000/kp9060Ajax.do";
...
var colModel01 = [
    				...
    				{name:'compName', index:'compName', width:'150px', align:'CENTER', columntype:'text', sortable:false, classes:'grid-col-TEXT'}
    				];
var groupHeaders01 = [];
...
function fnSelect(rowId) {
	...
	if (selRowId) {
		...
		opener.fnSetKp9060<%=("".equals(no)?"":"_"+no)%>(obj);
		window.close();
	} else {
		alert("업체를 선택해 주세요");
		return;
	...
	<tr>
		<th>
			<select id="searchGubun" name="searchGubun" style="max-width:80px;">
				<option value="1" <%="1".equals(cmRequest.getString("searchGubun","1"))?"selected":""%>>업체명</option>
				<option value="2" <%="2".equals(cmRequest.getString("searchGubun"))?"selected":""%>>연락처</option>
				<option value="3" <%="3".equals(cmRequest.getString("searchGubun"))?"selected":""%>>주소</option>
			</select>
		</th>
		<td>
