...
<%
String pageTitle = "자산조회";
String curAction = "/kp9000/kp9080.do";
String curGridAction = "/kp9000/kp9080Ajax.do";
...
var colNames01 = [
				'assetSeq','assetEname',
				'자산코드','품목명','상세명칭','취득일자','취득가액','잔존가액','위치','사용자'
				];
var colModel01 = [
   				{name:'assetSeq', index:'assetSeq', width:'0px', hidden:true},
   				{name:'assetEname', index:'assetEname', width:'0px', hidden:true},
...
function fnSelect(rowId) {
	...
	if (selRowId) {
		...
		opener.fnSetKp9080<%=("".equals(no)?"":"_"+no)%>(obj);
		window.close();
	} else {
		alert("자산을 선택해 주세요");
		return;
...
<body>
	...
	<div style="border:1px solid gray;background-color:#FFE08C;padding:10px;margin-top:10px">
		<span style="font-weight:bold;">자산을 조회하여 선택 버튼을 누르거나 더블 클릭해주세요.</span>
	</div>
	...
	<tr>
		<th>
			<select id="searchGubun" name="searchGubun" style="max-width:80px;">
				...
				<option value="7" <%="7".equals(cmRequest.getString("searchGubun"))?"selected":""%>>외자관세물품번호</option>
			</select>
		</th>
		<td>
			<input type="text" id="searchKeyword" name="searchKeyword" value="<%=cmRequest.getString("searchKeyword")%>" class="" />
