...
<%
String pageTitle = "사용자선택";
String curAction = "/kp9000/kp9030.do";
String curGridAction = "/kp9000/kp9030Ajax.do";
...
var colNames01 = [
                  '사용자번호','사번','성명','부서번호','부서','재직상태'
                  ];
var colModel01 = [
    				{name:'userNo', index:'userNo', width:'0px', hidden:true},
    				{name:'empNo', index:'empNo', width:'80px', align:'CENTER', columntype:'text', sortable:false, classes:'grid-col-TEXT'},
    				{name:'userName', index:'userName', width:'120px', align:'CENTER', columntype:'text', sortable:false, classes:'grid-col-TEXT'},
    				{name:'deptNo', index:'deptNo', width:'0px', hidden:true},
    				{name:'deptName', index:'deptName', width:'200px', align:'CENTER', columntype:'text', sortable:false, classes:'grid-col-TEXT'},
    				{name:'userType', index:'userType', width:'100px', align:'CENTER', columntype:'text', sortable:false, classes:'grid-col-TEXT'}
    				];
var groupHeaders01 = [];
...
function fnSelect(rowId) {
	...
	if (selRowId) {
		...
		opener.fnSetKp9030<%=("".equals(no)?"":"_"+no)%>(obj);
		window.close();
	} else {
		alert("사용자를 선택해 주세요");
		return;
...
<body>
	...
	<div style="border:1px solid gray;background-color:#FFE08C;padding:10px;margin-top:10px">
		<span style="font-weight:bold;">사용자를 검색하여 선택 버튼을 누르거나 더블 클릭해주세요.</span>
	</div>
	...
	<tr>
		<th>
			<select id="searchGubun" name="searchGubun" style="width:80px;">
				<option value="1" <%="1".equals(cmRequest.getString("searchGubun","1"))?"selected":""%>>성명</option>
				<option value="2" <%="2".equals(cmRequest.getString("searchGubun"))?"selected":""%>>사번</option>
				<option value="3" <%="3".equals(cmRequest.getString("searchGubun"))?"selected":""%>>부서</option>
			</select>
		</th>
		<td>
